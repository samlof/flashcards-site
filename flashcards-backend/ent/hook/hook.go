...
// The CardScheduleFunc type is an adapter to allow the use of ordinary
// function as CardSchedule mutator.
type CardScheduleFunc func(context.Context, *ent.CardScheduleMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CardScheduleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.CardScheduleMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CardScheduleMutation", m)
	}
	return f(ctx, mv)
}
